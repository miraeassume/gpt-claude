...
LLP      = 23      # 최종관찰만기
CP       = 60      # 최초수렴시점
LTFR     = 4.30  # 장기선도금리 4.30% (참고용 — SW Step2에서는 UFR_SW2 사용)
UFR_SW2  = 0.0     # Step2 SmithWilson + Alpha 교정 UFR (Excel U7=0)
...
def run():
    ...
    for p in range(len(t0_tenors)):
        ...
        uv_rows.append({
            ...
            "U_Spot_Disc(%)":   round(float(U_all[p]) * 100, 6),
            "V_Fwd_Disc(%)":    (round(float(V_all[p]) * 100, 6)
